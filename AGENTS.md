...
- `npm install`
- Copy [`.env.example`](.env.example) to **`.env`** and set **`CURSOR_API_KEY`**. Root [`src/load-env.ts`](src/load-env.ts) loads `.env` from the repo root. For cloud template runs you also need a repo URL (`--repo` or **`CURSOR_CLOUD_REPO_URL`**).
